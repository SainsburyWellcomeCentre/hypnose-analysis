...
| **4a moves — metric side COMPLETE** | `07182bd`..`9ebb031` 2026-08-06 | | **All 24 checklist metrics now exist in `metric_analysis`, D0 is complete for every tier, and both deliberate output changes are done.** Landed: tier-2 cores onto `position_data` (each filtering on the provenance flag for the blob it read, since the three blobs carry different positions); checklist 1-9 and 17-22 as `NEW` metrics; 23-24 into `compute_sing_rew_rates`; one `fa_port_counts` replacing 8 recomputes (finding 1); three exact DEDUPs; Q5 definitions B and C deleted; step 6; `compute_real_time_offset` (finding 15). **Two output changes, not one** — the plan assumed step 6 was the only one, but 23/24 add keys to the saved `sing_rew_metrics` dict; confirmed and regenerated separately (`--generate 057:20260709`, then all 9 for step 6), each with the metric-key diff confirming only the intended keys moved. **New QC tool: `qc/plot_regression.py`** — an old-vs-new diff of what the plotters *draw*, closing the gap this plan notes ("visual output needs eyeballing"). It immediately caught a `NameError` that a bare `except` would have swallowed into silently-empty output. **What remains is plot-side repointing only** — see "Remaining" in `docs/metric_audit.md`; no metric can be lost by it, and `plot_regression.py` gates it |
