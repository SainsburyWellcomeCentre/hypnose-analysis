...
| **4a moves — NOT FINISHED** | `07182bd`..`9ebb031` 2026-08-06 | | **Half of 4a. The "lose no metric" gate is met — all 24 checklist metrics exist in `metric_analysis`, D0 is complete for every tier, both deliberate output changes are done — but 4a's actual goal is *`visualization/` rid of all metric math*, and 21 recomputes are still there.** Landed: tier-2 cores onto `position_data` (each filtering on the provenance flag for the blob it read, since the three blobs carry different positions); checklist 1-9 and 17-22 as `NEW` metrics; 23-24 into `compute_sing_rew_rates`; one `fa_port_counts` replacing 8 recomputes (finding 1); three exact DEDUPs; Q5 definitions B and C deleted; step 6; `compute_real_time_offset` (finding 15). **Two output changes, not one** — this plan assumed step 6 was the only one, but 23/24 add keys to the saved `sing_rew_metrics` dict; confirmed and regenerated separately (`--generate 057:20260709`, then all 9 for step 6), each with the metric-key diff confirming only the intended keys moved. **New QC tool: `qc/plot_regression.py`** — an old-vs-new diff of what the plotters *draw*, closing the gap this plan notes ("visual output needs eyeballing"); it immediately caught a `NameError` a bare `except` would have swallowed into silently-empty output. **The 21 remaining items are itemised in `docs/metric_audit.md` → "Remaining — the 4a completion checklist".** Do not start 4b until they are done |
