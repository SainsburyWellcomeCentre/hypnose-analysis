...
class RewardAnalyser:
            ...
            if event_frames:
                try:
                    ...
                    if int(detect_stage(root)) > 7:
                        required_columns = ['timestamp', 'r1_poke', 'r2_poke', 'r1_olf_valve', 'r2_olf_valve', 'odourC_olf_valve', 'odourD_olf_valve', 'odourE_olf_valve', 'odourF_olf_valve', 'odourG_olf_valve', 'r1_olf_valve_off', 'r2_olf_valve_off', 'odourC_olf_valve_off', 'odourD_olf_valve_off', 'odourE_olf_valve_off', 'odourF_olf_valve_off', 'odourG_olf_valve_off', 'EndInitiation']
                    else:
                        required_columns = ['timestamp', 'r1_poke', 'r2_poke', 'r1_olf_valve', 'r2_olf_valve', 'r1_olf_valve_off', 'r2_olf_valve_off', 'EndInitiation']
                    ...
                    has_valve_events = (('r1_olf_valve' in all_events_df.columns and any(all_events_df['r1_olf_valve'])) or
                                        ...
                                        ('odourr1_olf_valve_off' in all_events_df.columns and any(all_events_df['r1_olf_valve_off'])) or 
                                        ('odourr2_olf_valve_off' in all_events_df.columns and any(all_events_df['r2_olf_valve_off'])) or 
    ...
    def get_decision_specificity(data_path):
        ...
        return session_data.get('specificity', {'C_nonspecificity': 0,
                        'D_nonspecificity': 0, 'E_nonspecificity': 0,
                        'F_nonspecificity': 0, 'G_nonspecificity': 0, 
                        'overall_nonspecificity': 0
                    })
...
def calculate_overall_decision_specificity_freerun(events_df): # TODO
    ...
    C_nonspecificity = ((C1_poke + C2_poke) / C_total * 100) if C_total else 0
    D_nonspecificity = ((D1_poke + D2_poke) / D_total * 100) if D_total else 0
    E_nonspecificity = ((E1_poke + E2_poke) / E_total * 100) if E_total else 0
    F_nonspecificity = ((F1_poke + F2_poke) / F_total * 100) if F_total else 0
    G_nonspecificity = ((G1_poke + G2_poke) / G_total * 100) if G_total else 0
    ...
    overall_nonspecificity = (nonR_pokes / nonR_trials * 100) if nonR_trials > 0 else 0

    return {
        'C_nonspecificity': C_nonspecificity,
        'D_nonspecificity': D_nonspecificity,
        'E_nonspecificity': E_nonspecificity,
        'F_nonspecificity': F_nonspecificity,
        'G_nonspecificity': G_nonspecificity,
        'overall_nonspecificity': overall_nonspecificity
    }
...
def calculate_overall_response_time(events_df):
    ...
    for e, end_idx in enumerate(end_initiation_indices):
        ...
        closest_valve_idx = None
        trial_type = None
        # for i in range(end_idx + 1, len(events_df)):
        for i in range(end_idx - 1, -1, -1):  # valve off happens after EndInitiation
            if events_df.loc[i, 'r1_olf_valve']:
                if events_df.loc[i+1, 'r1_olf_valve_off']:
                    closest_valve_idx = i
                trial_type = 'r1'
                trial_id[e,0] = 1
                break
        ...
        for j in range(end_idx + 1, len(events_df)):
            if events_df.loc[j, 'r1_poke'] or events_df.loc[j, 'r2_poke']:
                ...
                response_time = (poke_time - valve_time).total_seconds()
