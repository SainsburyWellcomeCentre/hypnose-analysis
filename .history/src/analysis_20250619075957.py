...
def calculate_overall_response_time(events_df):
    ...
    for e, end_idx in enumerate(end_initiation_indices):
        ...
        trial_type = None
        for i in range(end_idx - 1, -1, -1):  
            if events_df.loc[i, 'r1_olf_valve'] or events_df.loc[i, 'r1_olf_valve_off']:
                closest_valve_idx = i
                trial_type = 'r1'
                trial_id[e,0] = 1
                break
            elif events_df.loc[i, 'r2_olf_valve'] or events_df.loc[i, 'r2_olf_valve_off']:
                closest_valve_idx = i
                trial_type = 'r2'
                trial_id[e,0] = 2
                break
        ...
        for i in range(end_idx - 1, -1): 
            if events_df.loc[i, 'odour_poke_off']:
                closest_offset_idx = i
                break
