...
def analyze_session_folder(session_folder, reward_a=8.0, reward_b=8.0, verbose=False):
    ...
    # Variables to track overall session TODO 
    ...
    all_C_pokes = [], all_C_trials = []
    all_D_pokes = [], all_D_trials = []
    all_E_pokes = [], all_E_trials = []
    all_F_pokes = [], all_F_trials = []
    all_G_pokes = [], all_G_trials = []
    # all_C_false_alarm = []
    # all_D_false_alarm = []
    # all_E_false_alarm = []
    # all_F_false_alarm = []
    # all_G_false_alarm = []
    # all_overall_false_alarm = []
    ...
    for session_dir in sorted(session_dirs):
        ...
        if specificity_data:
            ...
            # all_C_false_alarm.append(specificity_data['C_false_alarm'])
            # all_D_false_alarm.append(specificity_data['D_false_alarm'])
            # all_E_false_alarm.append(specificity_data['E_false_alarm'])
            # all_F_false_alarm.append(specificity_data['F_false_alarm'])
            # all_G_false_alarm.append(specificity_data['G_false_alarm'])
            # all_overall_false_alarm.append(specificity_data['overall_false_alarm'])
    ...
    all_r1_correct_rt = np.array(all_r1_correct_rt).flatten()
    all_r1_incorrect_rt = np.array(all_r1_incorrect_rt).flatten()
    all_r2_correct_rt = np.array(all_r2_correct_rt).flatten()
    all_r2_incorrect_rt = np.array(all_r2_incorrect_rt).flatten()
    all_rt = np.array(all_rt).flatten()
