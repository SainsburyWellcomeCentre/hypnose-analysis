...
def calculate_overall_decision_specificity(events_df): # TODO
    """
...
def calculate_overall_response_time(events_df):
    ...
    for e, end_idx in enumerate(end_initiation_indices):
        ...
        closest_valve_idx = None
        trial_type = None
        for i in range(end_idx, len(end_initiation_indices)):
        # for i in range(end_idx - 1, -1, -1):  # valve off happens after EndInitiation
            if events_df.loc[i, 'r1_olf_valve_off']:
                closest_valve_idx = i
                trial_type = 'r1'
                trial_id[e,0] = 1
                break
            elif events_df.loc[i, 'r2_olf_valve_off']:
                closest_valve_idx = i
                trial_type = 'r2'
                trial_id[e,0] = 2
                break
