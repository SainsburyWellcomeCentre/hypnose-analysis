            ...
            if not olfactometer_valves_1_abs.empty and 'Valve1' in olfactometer_valves_1_abs.columns:
                try:
                    ...
                    if not odourF.empty:
                        ...
                        # Find OFF events
                        olfactometer_valves_1_shift = olfactometer_valves_1_abs.shift(1)
                        odourF_prev = olfactometer_valves_1_shift[olfactometer_valves_1_shift['Valve1'] == True].copy()
                        # odourF_prev = odourF_prev[['Time']].copy()
                        # Find transitions from ON to OFF
                        odourF_test = olfactometer_valves_1_abs[olfactometer_valves_1_abs['Valve1'] == True].copy()

                        off_after_on_mask = (odourF_prev == True) & (odourF_test == False)
                        odourF_off = olfactometer_valves_1_abs.loc[off_after_on_mask, ['Time']].copy()
                        odourF_off['odourF_olf_valve_off'] = True
                        event_frames.append(odourF_off)

                        print(f"Detected {len(odourF_off)} odour F OFF events after ON")
            ...
            if event_frames:
                try:
                    ...
                    if int(detect_stage(root)) > 7:
                        required_columns = ['timestamp', 'r1_poke', 'r2_poke', 'r1_olf_valve', 'r2_olf_valve', 'odourC_olf_valve', 'odourD_olf_valve', 'odourE_olf_valve', 'odourF_olf_valve', 'odourG_olf_valve', 'EndInitiation']
                    else:
                    ...
                    has_valve_events = (('r1_olf_valve' in all_events_df.columns and any(all_events_df['r1_olf_valve'])) or
                                        ...
                                        ('odourG_olf_valve' in all_events_df.columns and any(all_events_df['odourG_olf_valve'])))
