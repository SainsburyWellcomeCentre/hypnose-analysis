...
def plot_response_time(results_df, output_file=None, subject_id=None):
    ...
    r1_correct_rt = [results_df['total_r1_correct_rt'][session][0] for session in range(len(results_df['total_r1_correct_rt']))]
    r2_correct_rt = [results_df['total_r2_correct_rt'][session][0] for session in range(len(results_df['total_r2_correct_rt']))]
    r1_incorrect_rt = [results_df['total_r1_incorrect_rt'][session][0] for session in range(len(results_df['total_r1_incorrect_rt']))]
    r2_incorrect_rt = [results_df['total_r2_incorrect_rt'][session][0] for session in range(len(results_df['total_r2_incorrect_rt']))]
    ...
    ax.scatter(r1_correct_trials, r1_correct_rt, label='R1 correct RT', color='blue')
    ax.scatter(r2_correct_trials, r2_correct_rt, label='R2 correct RT', color='red')
    ax.scatter(r1_incorrect_trials, r1_incorrect_rt, label='R1 incorrect RT', color='cyan')
    ax.scatter(r2_incorrect_trials, r2_incorrect_rt, label='R2 incorrect RT', color='orange')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sessions", "-s", default=np.arange(50,54,1), help="List of session IDs (optional)")
