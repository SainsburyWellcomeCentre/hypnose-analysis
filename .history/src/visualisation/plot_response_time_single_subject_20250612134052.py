...
def plot_response_time(results_df, plot_file=None, subject_id=None):
    ...
    # valid_trials = np.concatenate([r1_correct_trials, r2_correct_trials, r1_incorrect_trials, r2_incorrect_trials])
    # invalid_trials = np.setdiff1d(valid_trials, offsets)
    ...
    r1_correct_rt = np.concatenate([results_df['total_r1_correct_rt'][session][0] \
                                    if results_df['total_r1_correct_rt'][session] else 0 \
    ...
    r2_correct_rt = np.concatenate([results_df['total_r2_correct_rt'][session][0] \
                                    if results_df['total_r2_correct_rt'][session] else 0 \
    ...
    r1_incorrect_rt = np.concatenate([results_df['total_r1_incorrect_rt'][session][0] \
                                    if results_df['total_r1_incorrect_rt'][session] else 0 \
    ...
    r2_incorrect_rt = np.concatenate([results_df['total_r2_incorrect_rt'][session][0] \
                                    if results_df['total_r2_incorrect_rt'][session] else 0 \
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        sys.argv.append("/Volumes/harris/hypnose/rawdata/sub-020_id-072/ses-31_date-20250506")
