...
def plot_false_alarms(results_df, output_file=None, subject_id=None):
    ...
    plt.scatter(results_df['date'], results_df['total_overall_false_alarm'], 
               label='Overall False Alarms', marker='o', s=60, color='black')
    ...
    plt.ylim(0, 1.05)  # Assuming false alarm is a proportion between 0 and 1
    ...
    for i, row in results_df.iterrows():
        plt.annotate(f"Ses-{row['session_id']}", 
                    (row['date'], row['total_overall_false_alarm']),
    ...
    for _, row in results_df.iterrows():
        if pd.notna(row['total_overall_false_alarm']):
            print(f"Session {row['session_id']} ({row['session_date']}): " 
                  f"Overall {row['total_overall_false_alarm']:.2%}, "
                  f"C {row['total_C_false_alarm']:.2%}, "
                  f"D {row['total_D_false_alarm']:.2%}, "
                  f"E {row['total_E_false_alarm']:.2%}, "
                  f"F {row['total_F_false_alarm']:.2%}, "
                  f"G {row['total_G_false_alarm']:.2%}")
