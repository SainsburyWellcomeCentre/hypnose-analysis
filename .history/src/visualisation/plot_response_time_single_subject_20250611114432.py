...
def plot_response_time(results_df, output_file=None, subject_id=None):
    ...
    # Create the plot
    _, ax = plt.subplots(4, 1, sharex=True, figsize=(12,6))
    ax = ax.ravel()

    ax[0].scatter(r1_correct_trials, r1_correct_rt, label='R1 correct RT', size=1, color='blue')
    ax[1].scatter(r2_correct_trials, r2_correct_rt, label='R2 correct RT', size=1, color='red')
    ax[2].scatter(r1_incorrect_trials, r1_incorrect_rt, label='R1 incorrect RT', size=1, color='cyan')
    ax[3].scatter(r2_incorrect_trials, r2_incorrect_rt, label='R2 incorrect RT', size=1, color='orange')

    # Add average response time
    ax[0].plot(np.concatenate(results_df['avg_response_time']), label='avg response time', color='black', linestyle='--', alpha=0.7)
    ax[1].plot(np.concatenate(results_df['avg_response_time']), label='avg response time', color='black', linestyle='--', alpha=0.7)
    ax[2].plot(np.concatenate(results_df['avg_response_time']), label='avg response time', color='black', linestyle='--', alpha=0.7)
    ax[3].plot(np.concatenate(results_df['avg_response_time']), label='avg response time', color='black', linestyle='--', alpha=0.7)
    
    # Add session boundaries 
    ax[0].vlines(x=offsets[1:], ymin=np.min(r1_correct_rt), ymax=np.max(r1_correct_rt))
    ax[1].vlines(x=offsets[1:], ymin=np.min(r2_correct_rt), ymax=np.max(r2_correct_rt))
    ax[2].vlines(x=offsets[1:], ymin=np.min(r1_incorrect_rt), ymax=np.max(r1_incorrect_rt))
    ax[3].vlines(x=offsets[1:], ymin=np.min(r2_incorrect_rt), ymax=np.max(r2_incorrect_rt))

    # Format the plot
    ax[3].set_xlabel('Trial')
    ax[0].set_ylabel('Response time (s)')
    ax[1].set_ylabel('Response time (s)')
    ax[2].set_ylabel('Response time (s)')
    ax[3].set_ylabel('Response time (s)')
    
    # Set title with subject ID if provided
    if subject_id:
        plt.title(f'Response time - sub-{subject_id}')
    else:
        plt.title(f'Response time')
