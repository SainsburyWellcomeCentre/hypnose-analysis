...
def plot_response_time(results_df, plot_file=None, subject_id=None):
    ...
    all_avg_response_time = np.concatenate([results_df['avg_response_time'][session] \ 
                                            if results_df['all_avg_response_time'][session] else np.zeros(len(sublist[0]))))
        ...
        for (session_id, session_date), session_paths in sorted(grouped_sessions.items()):
            ...
            for session_path in session_paths:
                ...
                if len(total_rt) > 0:
                    ...
                    avg_response_time = np.zeros(total_trial_id)
