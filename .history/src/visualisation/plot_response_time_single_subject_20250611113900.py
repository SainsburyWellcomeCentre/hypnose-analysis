...
def plot_response_time(results_df, output_file=None, subject_id=None):
    ...
    # Create the plot
    _, ax = plt.subplots(4, 1, sharex=True, sharey=True, figsize=(12,6))
    ax = ax.ravel()

    ax[0].scatter(r1_correct_trials, r1_correct_rt, label='R1 correct RT', color='blue')
    ax[1].scatter(r2_correct_trials, r2_correct_rt, label='R2 correct RT', color='red')
    ax[2].scatter(r1_incorrect_trials, r1_incorrect_rt, label='R1 incorrect RT', color='cyan')
    ax[3].scatter(r2_incorrect_trials, r2_incorrect_rt, label='R2 incorrect RT', color='orange')

    # Add average response time
    ax[0].plot(results_df['avg_response_time'], label='avg response time', color='black', linestyle='--', alpha=0.7)
    ax[1].plot(results_df['avg_response_time'], label='avg response time', color='black', linestyle='--', alpha=0.7)
    ax[2].plot(results_df['avg_response_time'], label='avg response time', color='black', linestyle='--', alpha=0.7)
    ax[3].plot(results_df['avg_response_time'], label='avg response time', color='black', linestyle='--', alpha=0.7)
